...
type ParallelTasksDAG() =
    ...
    let verifyTopologicalSort (tasks: Dictionary<int, TaskInfo>) =
        ...
        let tasksToFrom =
            new Dictionary<int, MList<int>>(tasks.Values.Count, HashIdentity.Structural)
        ...
        for op in tasks.Values do
            ...
            for deptId in op.Edges do
                ...
                if not <| success then
                    tasksFromTo.Add(deptId, new MList<int>())
    ...
    let rec getDependentOperation (dep: int list) (ops: Dictionary<int, TaskInfo>) acc =
        ...
        | [] -> acc
        | h :: t ->
            ops.[h] <- { ops.[h] with EdgesLeft = nrd ops.[h].EdgesLeft }
    ...
    let dagAgent =
        let inbox =
            new MailboxProcessor<TaskMessage>(fun inbox ->
                let rec loop
                    (tasks: Dictionary<int, TaskInfo>) // #D
                    ...
                    async {
                        ...
                        | ExecuteTasks -> // #E
                            ...
                            for KeyValue (key, value) in tasks do // #F
                                let operation = { value with EdgesLeft = Some(value.Edges.Length) }
